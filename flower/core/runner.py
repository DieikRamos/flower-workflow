...
class FlowerRunner:
    ...
    def run(self):
        ...
        while pending_steps:
            ...
            for key, step in pending_steps:
                if not step.depends or all(item in executed_steps for item in step.depends):
                    current_steps.append((key, step))

            pending_steps = [item for item in pending_steps if item not in current_steps]
            args = [(key, step, context, workflow_params) for key, step in current_steps]
    ...
    def __run_step(self, key, step, context, workflow_params):
        ...
        if action:
            ...
            context[key] = action(context=context, workflow_context=workflow_params, params=parsed_params)
        else:
            sub_workflow = self.schema.workflows.get(step.action)
            if sub_workflow:
                sub_flow_runner = FlowerRunner(self.schema, step.action, workflow_params)
                context[key] = sub_flow_runner.run()
